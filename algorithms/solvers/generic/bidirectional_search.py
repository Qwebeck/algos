...
from algorithms.base.heuristic import Heuristic, NoHeuristic
from algorithms.base.problem import ReversibleProblem, Problem
from algorithms.base.state import State
from algorithms.solvers.utils import PriorityQueue
from algorithms.tree.node import Node
from algorithms.tree.tree import NodeEventSubscriber, Tree
...
class SearchProcess():
        ...
        if not opposite.has_scanned_state(candidate.state):
            for node in self.tree.expand(self.problem, candidate):
                ...
                if not self.has_labeled_state(node.state) or self.labeled_state_cost(node.state) > node.cost:
                    ...
                    upper_bound = self._update_upper_bound(
                        node, upper_bound, opposite)
    ...
    def _select_candidate(self,
        ...
        while self.frontier and not chosen:
            ...
            cost_through_state = self.labeled_state_cost(
                candidate.state) + opposite.cost_lower_bound - opposite.heuristic(candidate.state)

            if not opposite.has_scanned_state(
                    candidate.state) and upper_bound < float('inf') and cost_through_state >= upper_bound:
                self.rejected_states.add(candidate.state)
                continue
    ...
    def _update_upper_bound(self, node: Node, upper_bound_cost: float, other_process: 'SearchProcess') -> float:
        ...
        new_upper_bound_cost = self.labeled_state_cost(
            node.state) + other_process.labeled_state_cost(node.state)
